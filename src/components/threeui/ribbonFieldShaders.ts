// Adapted from MengTo/threeui Ribbon Field. MIT; see adjacent LICENSE and README.
...
export const RIBBON_FIELD_FRAGMENT_SHADER = `
        ...
        void main() {
          ...
          // Champagne silk on warm ivory, instead of the original neon dot field.
          vec3 ivory = vec3(0.980, 0.973, 0.957);
          vec3 gold = vec3(0.77, 0.59, 0.36);
          float silk = clamp(r1 * 0.32 + r2 * 0.25 + r3 * 0.18, 0.0, 0.65);
          vec3 champagne = mix(ivory, gold, silk);
          champagne += bloom * 0.032;
          gl_FragColor = vec4(champagne, 1.0);
        }
      `;
